...
mock 必须打在节点**真正调用**的方法上，否则测试会真的联网：
  ...
  - TrendContext.execute_search          —— 搜索引擎
"""
...
def _fake_bocha_response():
    """引擎实际使用 Bocha 后端，返回体必须带 .webpages（Tavily 的 .results 已不被消费）。"""
    from TrendEngine.tools import BochaResponse, WebpageResult
...
_AGENT_CONFIG = {
    "TREND_ENGINE_API_KEY": "sk-test-fake-key",
    "TREND_ENGINE_MODEL_NAME": "test-model",
    "TREND_ENGINE_BASE_URL": "https://test.api.com",
...
@pytest.fixture
def agent():
    """创建 mock 好外部依赖的 TrendEngine runner。"""
    from types import SimpleNamespace

    from TrendEngine.agent import run_research
    from TrendEngine.llms import LLMClient
    from TrendEngine.tools import BochaMultimodalSearch
    from TrendEngine.utils.config import Settings

    config = Settings(OUTPUT_DIR="/tmp/test_trend_reports", **_AGENT_CONFIG)
    ...
    patches = [
        patch("TrendEngine.llms.base.LLMClient.structured_invoke", side_effect=_fake_structured),
        patch("TrendEngine.llms.base.LLMClient.stream_invoke_to_string", return_value=_FINAL_REPORT),
        patch("TrendEngine.context.TrendContext.execute_search", return_value=_fake_bocha_response()),
    ]
    for p in patches:
        p.start()
...
class TestTrendEngineBehavior:
    """TrendEngine 行为级端到端测试，只验证外部契约。"""
    ...
    def test_search_failure_propagates_error(self, agent):
        ...
        with patch("TrendEngine.context.TrendContext.execute_search", side_effect=RuntimeError("Tavily API unreachable")):
    ...
    def test_progress_callback_receives_events(self, agent):
        """progress_callback 在 research 期间被触发（不传回调时不应崩溃）。"""
        events = []
