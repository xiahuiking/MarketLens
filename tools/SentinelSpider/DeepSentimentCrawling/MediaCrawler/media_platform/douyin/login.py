...
class DouYinLogin(AbstractLogin):
    ...
    async def popup_login_dialog(self):
        ...
        try:
            ...
            # 页面崩溃（Page crashed / Target crashed）等严重错误，不应被当作"弹窗未出现"处理
            utils.logger.error(f"[DouYinLogin.popup_login_dialog] browser page crashed while waiting for login dialog: {e}")
            raise
