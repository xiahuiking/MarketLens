...
class DouYinCrawler(AbstractCrawler):
    ...
    async def launch_browser(
        ...
        # 稳定性参数：禁用 GPU 避免加载抖音等重页面时出现 "Page crashed / Target crashed" 的
        # 渲染进程崩溃问题。注意：不要加 --no-sandbox / --disable-setuid-sandbox，这两个是
        # Linux 专属参数，在 macOS 上会导致 "Mach rendezvous failed (parent died)" 启动崩溃。
        browser_args = [
            "--disable-gpu",
        ...
        if config.SAVE_LOGIN_STATE:
            ...
            browser = await chromium.launch(headless=headless, proxy=playwright_proxy, args=browser_args)  # type: ignore
