...
数据模型定义位置：
- SentinelSpider/DeepSentimentCrawling/MediaCrawler/schema/tables.sql  # 主表结构来源文件
- 本模块（自动映射SQL表，适配MySQL/PostgreSQL，推荐手动完善注释、唯一/索引补充）
- SentinelSpider/schema/models_sa.py  # Base 定义来源

本模块以SentinelSpider\DeepSentimentCrawling\MediaCrawler\database\models.py为准
"""
