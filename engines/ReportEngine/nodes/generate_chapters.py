...
from ..exceptions import ReportCancelledError
...
class GenerateChaptersNode(BaseNode):
    """
    负责按章节调用LLM并校验JSON结构。
    ...
    def __init__(self, ctx):
        ...
        """
        self.ctx = ctx
        super().__init__(ctx.llm_client, "GenerateChaptersNode")
        self.validator = ctx.validator
        self.storage = ctx.chapter_storage
        self.fallback_llm_clients: List[Tuple[str, Any]] = ctx.json_rescue_clients or [
            ("report_engine", ctx.llm_client)
        ]
        error_dir = Path(getattr(ctx.config, "JSON_ERROR_LOG_DIR", "logs/json_repair_failures"))
    ...
    def _build_payload(self, section: TemplateSection, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 章节篇幅规划（来自PlanBudgetNode），用于指导字数与强调点
    ...
    def _as_paragraph_block(text: str) -> Dict[str, Any]:
        ...
        return {
            "type": "paragraph",
            "inlines": [GenerateChaptersNode._as_inline_run(text)],
        }
...
def _finalize_sparse(chapter: dict) -> dict:
    ...
    warning = {
        ...
        "inlines": [
            {
                "text": "本章LLM生成的内容字数可能过低，必要时可以尝试重新运行程序。",
                "marks": [{"type": "italic"}],
            }
        ],
