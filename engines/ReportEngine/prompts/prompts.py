...
input_schema_html_generation = {
    ...
    "properties": {
        "query": {"type": "string"},
        "trend_engine_report": {"type": "string"},
        "competitor_engine_report": {"type": "string"},
        "review_engine_report": {"type": "string"},
...
chapter_generation_input_schema = {
    ...
    "properties": {
        ...
        "reports": {
            "type": "object",
            "properties": {
                "trend_engine": {"type": "string"},
                "competitor_engine": {"type": "string"},
                "review_engine": {"type": "string"}
            }
        },
...
选择标准：
1. 查询内容的主题类型（单商品口碑、多商品竞品对比、品类趋势、品类热点等）
2. 报告的紧急程度和时效性
3. 分析的深度和广度要求
4. 目标受众和使用场景

可用模板类型，推荐使用“商品口碑分析报告模板”：
- 商品口碑分析报告模板：当需要对单个商品或品牌在特定周期内的评分分布、评论情感、优缺点与差评归因进行深度评估与复盘时，应选择此模板。核心任务是商品级口碑的全面分析。
- 竞品对比分析报告模板：当目标是系统性地对比多个同类商品的价格、评分、口碑、卖点及用户反馈，以明确自身定位并制定差异化策略时，应选择此模板。核心任务是对比与洞察。
- 商品评论日常监测报告模板：当需要进行常态化、高频次（如每周、每月）的评论追踪，旨在快速掌握动态、呈现关键数据、并及时发现差评与风险苗头时，应选择此模板。核心任务是数据呈现与动态追踪。
- 品类趋势与行业洞察报告模板：当监测到品类热度、价格带分布或用户关注点发生足以影响选品与运营的趋势性变化时，应选择此模板。核心任务是趋势研判与品类洞察。
- 品类热点与爆款分析报告模板：当某品类出现引发广泛讨论的热点商品或爆款趋势时，应选择此模板。核心任务是洞察爆款成因与关联机会。
- 负面口碑与危机应对报告模板：当监测到与目标商品直接相关的、具有潜在危害的突发负面口碑或评分危机时，应选择此模板。核心任务是快速响应、评估风险、控制事态。
...
10. engineQuote 仅用于呈现单Agent的原话：使用 block.type="engineQuote"，engine 取值 review/competitor/trend，title 必须固定为对应Agent名字（review->口碑 Agent，competitor->竞品 Agent，trend->趋势 Agent，不可自定义），内部 blocks 只允许 paragraph，paragraph.inlines 的 marks 仅可使用 bold/italic（可留空），禁止在 engineQuote 中放表格/图表/引用/公式等；当 reports 或 forumLogs 中有明确的文字段落、结论、数字/时间等可直接引用时，优先分别从 口碑/竞品/趋势 三个 Agent 摘出关键原文或文字版数据放入 engineQuote，尽量覆盖三类 Agent 而非只用单一来源，严禁臆造内容或把表格/图表改写进 engineQuote。
